...
#Assign your FRED API Key (as str) to the fredkey variable below. You can get one here - https://research.stlouisfed.org/useraccount/login/secure/
fredkey = ''
...
#SQLite connection & custom db functions
...
#SQL operations
create_parameters = '''CREATE TABLE parameters (series_desc TEXT NOT NULL, 
...
insert_parameters = 'INSERT INTO parameters (series_desc, series_id, observation_start, units, frequency, aggregation_method) VALUES (?, ?, ?, ?, ?, ?)'

create_freddata = '''CREATE TABLE freddata (date TEXT NOT NULL, 
...
if chk_table('parameters') == 0:
    c.execute(create_parameters)
    conn.commit()

elif chk_table('parameters') == 1:
    pass


if chk_table('freddata') == 0:
    c.execute(create_freddata)
    conn.commit()
    
elif chk_table('freddata') == 1:
    pass
